                    ...
                    <tbody>
                        ...
                        document.getElementById('delete-attendee-{{ $attendee->id }}').submit();">
                                    ...
                                    <form id="delete-attendee-{{ $attendee->id }}"
                                        action="{{ route('attendees.destroy', ['attendee' => $attendee]) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
                                <td>Evento: {{ $attendee->event->name }}</td>
                                <td>Nombre: {{ $attendee->person->name }}</td>
                                <td>Apellido: {{ $attendee->person->last_name }}</td>
                            </tr>
                        @empty
                            <p>No Attendees</p>
                        @endforelse
